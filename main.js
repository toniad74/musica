...
function logout() {
    signOut(auth).then(() => {
        ...
        // Clear user data
        currentUserUid = null;
        localStorage.removeItem('amaya_playlists'); // Optional: clear local cache if desired
        playlists = [];
        renderPlaylists();
        // Reset UI
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        // (Assuming logic to merge/load exists in loadPlaylists, or we implement basic load here)
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            if (data.playlists) {
                ...
                savePlaylists(); // Sync to local storage
                renderPlaylists();
                showToast("Listas sincronizadas");
            }
        }
    } catch (e) {
