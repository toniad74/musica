...
// ==========================================
// IMAGE TO PLAYLIST (OCR Feature)
// ==========================================

let imgExtractedSongs = [];

function showImagePlaylistModal() {
    document.getElementById('imagePlaylistModal').classList.remove('hidden');
    document.getElementById('imgStep1').classList.remove('hidden');
    document.getElementById('imgStep2').classList.add('hidden');
    document.getElementById('imgStep3').classList.add('hidden');
    document.getElementById('imgStep4').classList.add('hidden');
...
function hideImagePlaylistModal() {
    document.getElementById('imagePlaylistModal').classList.add('hidden');
    imgExtractedSongs = [];
}
...
function setupImageDropZone() {
    ...
    newDropZone.addEventListener('dragover', (e) => {
        e.preventDefault();
        newDropZone.classList.add('border-green-500', 'bg-green-500/10');
    });

    newDropZone.addEventListener('dragleave', () => {
        newDropZone.classList.remove('border-green-500', 'bg-green-500/10');
    });
...
function handleImageFile(file) {
    const reader = new FileReader();
    reader.onload = (e) => {
        document.getElementById('imgPreviewImage').src = e.target.result;
...
async function processImageOCR() {
    ...
    document.getElementById('imgStep1').classList.add('hidden');
    document.getElementById('imgStep2').classList.remove('hidden');

    const imgSrc = document.getElementById('imgPreviewImage').src;
    const progressText = document.getElementById('imgProgressText');
    const progressDetail = document.getElementById('imgProgressDetail');

    try {
        progressText.textContent = 'Inicializando reconocimiento...';
        progressDetail.textContent = 'Cargando motor OCR';

        const worker = await Tesseract.createWorker('spa+eng', 1, {
            logger: (m) => {
                if (m.status === 'recognizing text') {
                    const pct = Math.round(m.progress * 100);
                    progressText.textContent = `Analizando imagen... ${pct}%`;
                    progressDetail.textContent = 'Reconociendo texto';
                }
            }
        });
        ...
        // Parse the text into song lines
        const lines = text.split('\n')
            .map(line => line.trim())
            .filter(line => {
                // Filter out empty lines, very short lines, and common non-song text
                if (line.length < 3) return false;
                if (/^(\d+[\.\)\-:]?\s*)$/.test(line)) return false; // Just numbers
                if (/^(playlist|lista|canciones|songs|album|álbum|track|pista)$/i.test(line)) return false;
                return true;
            })
            .map(line => {
        ...
        imgExtractedSongs = [...new Set(lines)]; // Remove duplicates

        if (imgExtractedSongs.length === 0) {
            showToast('No se detectaron canciones en la imagen. Intenta con una imagen más clara.', 'warning');
            document.getElementById('imgStep2').classList.add('hidden');
            document.getElementById('imgStep1').classList.remove('hidden');
            document.getElementById('imgPreview').classList.remove('hidden');
            document.getElementById('imgDropZone').classList.add('hidden');
...
function renderImgSongsList() {
    const container = document.getElementById('imgSongsList');
    document.getElementById('imgSongCount').textContent = imgExtractedSongs.length;

    container.innerHTML = imgExtractedSongs.map((song, i) => `
        <div class="flex items-center gap-2 bg-[#3e3e3e] rounded-lg px-3 py-2 group">
            <span class="text-gray-500 text-xs w-6">${i + 1}</span>
            <input type="text" value="${song.replace(/"/g, '&quot;')}" 
                class="flex-1 bg-transparent text-white text-sm outline-none focus:ring-1 focus:ring-green-500 rounded px-1"
                onchange="imgUpdateSong(${i}, this.value)">
            <button onclick="imgRemoveSong(${i})" class="text-red-400 hover:text-red-300 opacity-0 group-hover:opacity-100 transition-opacity">
...
function imgUpdateSong(index, value) {
    imgExtractedSongs[index] = value.trim();
}

function imgRemoveSong(index) {
    imgExtractedSongs.splice(index, 1);
    renderImgSongsList();
}

function imgAddManualSong() {
    imgExtractedSongs.push('');
    renderImgSongsList();
    // Focus the last input
    const inputs = document.querySelectorAll('#imgSongsList input');
    if (inputs.length > 0) inputs[inputs.length - 1].focus();
}

async function createPlaylistFromImage() {
    const name = document.getElementById('imgPlaylistName').value.trim();
    if (!name) {
        showToast('Escribe un nombre para la playlist', 'warning');
        document.getElementById('imgPlaylistName').focus();
        return;
    }

    // Filter empty songs
    const songs = imgExtractedSongs.filter(s => s.trim().length > 0);
    if (songs.length === 0) {
        showToast('No hay canciones para añadir', 'warning');
        return;
    }

    // Show building step
    document.getElementById('imgStep3').classList.add('hidden');
    document.getElementById('imgStep4').classList.remove('hidden');

    const buildProgress = document.getElementById('imgBuildProgress');
    const buildDetail = document.getElementById('imgBuildDetail');
    const foundSongs = [];
    let searched = 0;

    for (const songQuery of songs) {
        searched++;
        buildDetail.textContent = `${searched} / ${songs.length}`;
        buildProgress.textContent = `Buscando: ${songQuery.substring(0, 40)}...`;

        try {
            const result = await searchSongForPlaylist(songQuery);
            if (result) {
                foundSongs.push(result);
            }
        } catch (e) {
            console.warn(`Failed to find: ${songQuery}`, e);
        }

        // Small delay to avoid rate limiting
        if (searched < songs.length) {
            await new Promise(r => setTimeout(r, 300));
        }
    }

    if (foundSongs.length === 0) {
        showToast('No se encontró ninguna canción', 'error');
        document.getElementById('imgStep4').classList.add('hidden');
        document.getElementById('imgStep3').classList.remove('hidden');
        return;
    }
    ...
    const newPlaylist = {
        id: 'pl_' + Date.now(),
        name: name,
        description: `Creada desde imagen (${foundSongs.length}/${songs.length} encontradas)`,
        cover: APP_LOGO_URL,
        songs: foundSongs
    };
    ...
    savePlaylists();
    renderPlaylists();
    hideImagePlaylistModal();

    showToast(`Playlist "${name}" creada con ${foundSongs.length} canciones`, 'success');
}

async function searchSongForPlaylist(query) {
    const currentKey = getCurrentApiKey();

    // Try YouTube API first
    if (currentKey) {
        try {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&q=${encodeURIComponent(query + ' audio')}&type=video&videoCategoryId=10&key=${currentKey}`);
            const data = await response.json();
            ...
            if (data.items && data.items.length > 0) {
                ...
                // Try to get duration
                try {
                    const detResp = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${song.id}&key=${currentKey}`);
                    const detData = await detResp.json();
                    if (detData.items && detData.items[0]) {
                        song.duration = parseISO8601Duration(detData.items[0].contentDetails.duration);
                        song.durationSec = parseISO8601DurationInSeconds(detData.items[0].contentDetails.duration);
                    }
                } catch (e) { /* duration fetch failed, not critical */ }

                return song;
            }
        } catch (e) {
            console.warn('YouTube API search failed, trying Piped fallback');
        }
    }
    ...
    try {
        ...
        if (pipedResults && pipedResults.length > 0) {
            return pipedResults[0];
        }
    } catch (e) {
        console.warn('Piped fallback also failed for:', query);
    }
